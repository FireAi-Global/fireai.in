...
// Define the raw countries array.
const rawCountries = [
...
export const countriesWithFlags = rawCountries.map((country) => ({
    ...country,
    flag: codeToFlagEmoji(country.code),
}));

const defaultCountry =
    countriesWithFlags.find((country) => country.code === "IN") ||
    countriesWithFlags[0];

interface MobileNumberInputProps {
    value: string; // The full phone number (dial code + local number)
    onChange: (phone: string) => void;
    error?: string;
}

export default function MobileNumberInput(props: MobileNumberInputProps) {
    const [selectedCountry, setSelectedCountry] = createSignal(defaultCountry);
    const [localNumber, setLocalNumber] = createSignal("");

    createEffect(() => {
        const dialCode = selectedCountry().dial_code;
        if (props.value.startsWith(dialCode)) {
            setLocalNumber(props.value.slice(dialCode.length));
        }
    });

    const handleCountryChange = (e: Event) => {
        const newDialCode = (e.currentTarget as HTMLSelectElement).value;
        const country = countriesWithFlags.find((c) => c.dial_code === newDialCode);
        if (country) {
            setSelectedCountry(country);
            const fullNumber = country.dial_code + localNumber();
            props.onChange(fullNumber);
        }
    };

    const handleLocalNumberChange = (e: Event) => {
        const newLocal = (e.currentTarget as HTMLInputElement).value;
        if (/^[0-9]*$/.test(newLocal)) { // Only allow digits
            setLocalNumber(newLocal);
            const fullNumber = selectedCountry().dial_code + newLocal;
            props.onChange(fullNumber);
        }
    };
